...
while True:
  user_input = input("Please enter a manufacturer and item type or q to quit\n")
  if user_input == "q":
    break
  user_list = []
  consider_list = []
  for item in final_list:
     item_date = datetime.strptime(item[4],'%m/%d/%Y')
     if (item[1] in user_input) and (item[2] in user_input) and (item_date > curr) and (item[5] != 'damaged'):
       user_list.append(item)
     elif (item[2] in user_input) and (item_date > curr)  and (item[5] != 'damaged'):
       consider_list.append(item)
  if user_list:
    user_item = max(user_list, key=lambda x: x[3])
    print('Your item is:',user_item)
    price = int(user_item[3])
    if consider_list:
      print('You may, also, consider:',consider_list[min(range(len(consider_list)), key = lambda i: abs( int(consider_list[i][3]) - price))])
  else:
